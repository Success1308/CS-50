...
bool dfs(int winner, int loser, bool visited[])
{
    if (winner == loser)
    {
        return true;
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[winner][i] && !visited[i])
        {
            if (dfs(i, loser, visited))
            {
                return true;
            }
        }
...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        // Check for cycle before locking the pair
        bool visited[candidate_count];
        memset(visited, false, sizeof(visited));

        if (!dfs(pairs[i].loser, pairs[i].winner, visited))
        {
